...
@export var perlin_noise_water:FastNoiseLite
@export var perlin_noise_mountain:FastNoiseLite
...
func _ready() -> void:
	perlin_noise_water.seed = randi()
	perlin_noise_mountain.seed = randi()
	...
	for i in range(-render_range, render_range):
		for j in range(-render_range,render_range):
			...
			var perlin_value_water = abs(perlin_noise_water.get_noise_2d(i+j*0.5,j*sqrt(3)/2))
			var perlin_value_land = abs(perlin_noise_mountain.get_noise_2d(i+j*0.5,j*sqrt(3)/2))
			var tile_inx:int = 1
			var distance:float = hex_len(Vector2i(i,j))
			if perlin_value_land > mountain_threshold*exp(-(distance/range_factor)**2):
				tile_inx = 2
			elif perlin_value_water > water_threshold*exp(-(distance/range_factor)**2):
				tile_inx = 0
